...
from models.DANet.submodule import *
from models.DANet.CBAM import CBAMBlock
from models.DANet.DisparityAttention import DABlock
import math
...
def DANet_G(d):
    return DANet(d, use_concat_volume=False)


def DANet_GC(d):
    return DANet(d, use_concat_volume=True)


if __name__ == '__main__':
    left = torch.randn(1, 3, 256, 512)  # batch size=1, channel=2, height=256, width=512
    right = torch.randn(1, 3, 256, 512)
    kernel_size = left.shape[2]  # kernel_size = input.height
    danet = DANet(maxdisp=256, use_concat_volume=False)
    output = danet(left, right)
