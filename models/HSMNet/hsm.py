...
class HSMNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            return pred3
        # torch.squeeze(entropy)


if __name__ == '__main__':
    left = torch.randn(1, 3, 608, 1056).cuda()  # batch size=1, channel=3, height=608, width=1056
    right = torch.randn(1, 3, 608, 1056).cuda()
    hsmnet = HSMNet(maxdisp=256, clean=False, level=1)
    hsmnet = hsmnet.cuda()
    with torch.inference_mode():
        output = hsmnet(left, right)
