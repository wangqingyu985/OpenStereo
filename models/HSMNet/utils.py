...
class unet(nn.Module):
    def __init__(self):
        ...
        self.res_block3 = self._make_layer(block=residualBlock, planes=64, blocks=1, stride=2)
        self.res_block5 = self._make_layer(block=residualBlock, planes=128, blocks=1, stride=2)
        self.res_block6 = self._make_layer(block=residualBlock, planes=128, blocks=1, stride=2)
        self.res_block7 = self._make_layer(block=residualBlock, planes=128, blocks=1, stride=2)
        self.pyramid_pooling = pyramidPooling(in_channels=128, pool_sizes=None, fusion_mode='sum', model_name='icnet')
        # Iconvs
    ...
    def forward(self, x):
        # H, W -> H/2, W/2
        # x: [2*batch_size, channel=3, H, W]
        conv1_1 = self.convbnrelu1_1(x)  # conv1_1: [2*batch_size, channel=16, H/2, W/2]
        conv1_2 = self.convbnrelu1_2(conv1_1)  # conv1_1: [2*batch_size, channel=16, H/2, W/2]
        conv1_3 = self.convbnrelu1_3(conv1_2)  # conv1_1: [2*batch_size, channel=32, H/2, W/2]
        conv1 = conv1_3  # conv1: [2*batch_size, channel=32, H/2, W/2]

        # H/2, W/2 -> H/4, W/4
        pool1 = F.max_pool2d(conv1, 3, 2, 1)  # pool1: [2*batch_size, channel=32, H/4, W/4]

        # H/4, W/4 -> H/16, W/16
        conv3 = self.res_block3(pool1)  # conv3: [2*batch_size, channel=64, H/8, W/8]
        conv4 = self.res_block5(conv3)  # conv4: [2*batch_size, channel=128, H/16, W/16]
        conv5 = self.res_block6(conv4)  # conv5: [2*batch_size, channel=128, H/32, W/32]
        conv6 = self.res_block7(conv5)  # conv6: [2*batch_size, channel=128, H/64, W/64]
        conv6 = self.pyramid_pooling(conv6)  # conv6: [2*batch_size, channel=128, H/64, W/64]

        concat5 = torch.cat((conv5, self.upconv6(conv6)), dim=1)  # concat5: [2*batch_size, channel=128+64, H/32, W/32]
        conv5 = self.iconv5(concat5)  # conv5: [2*batch_size, channel=128, H/32, W/32]

        concat4 = torch.cat((conv4, self.upconv5(conv5)), dim=1)  # concat4: [2*batch_size, channel=128+64, H/16, W/16]
        conv4 = self.iconv4(concat4)  # conv4: [2*batch_size, channel=128, H/16, W/16]

        concat3 = torch.cat((conv3, self.upconv4(conv4)), dim=1)  # concat3: [2*batch_size, channel=64+64, H/8, W/8]
        conv3 = self.iconv3(concat3)  # conv3: [2*batch_size, channel=64, H/8, W/8]

        proj6 = self.proj6(conv6)  # proj6: [2*batch_size, channel=32, H/64, W/64]
        proj5 = self.proj5(conv5)  # proj5: [2*batch_size, channel=16, H/32, W/32]
        proj4 = self.proj4(conv4)  # proj4: [2*batch_size, channel=16, H/16, W/16]
        proj3 = self.proj3(conv3)  # proj3: [2*batch_size, channel=16, H/8, W/8]
...
class conv2DBatchNormRelu(nn.Module):
    def __init__(self, in_channels, n_filters, k_size,  stride, padding, bias=True, dilation=1, with_bn=True):
        ...
        if dilation > 1:
            conv_mod = nn.Conv2d(in_channels=int(in_channels), out_channels=int(n_filters), kernel_size=k_size,
                                 padding=padding, stride=stride, bias=bias, dilation=dilation)
        else:
            conv_mod = nn.Conv2d(in_channels=int(in_channels), out_channels=int(n_filters), kernel_size=k_size,
                                 padding=padding, stride=stride, bias=bias, dilation=1)
